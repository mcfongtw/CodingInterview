...
\section{Pow(x, n) / Leet Code 50 / Medium}
...
\subsection{Solution - Dynamic Programming (Bottom-up Iteration)}
#### Walkthrough\\
...
\subsection{Java Code - Dynamic Programming (Bottom-up Iteration)}
```java
int fib(int n) {
    if(n == 0) {
        return 0;
...
### Walkthough - Dynamic Programming with Memoization
...
#### Algorithm\\
 memo \@ref(memo), recursive \@ref(recursive), dp \@ref(dp)
