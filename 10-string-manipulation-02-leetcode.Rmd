...
## Integer to Roman {#lc-integer-to-roman}
...
- **Tags**:
- **Techniques**: [Hash Table](#glossary-hash-table), [Simulation](#glossary-simulation)
...
- If the value starts with 4 or 9 use the **subtractive form** representing one symbol subtracted from the following symbol:
  ...
  - 40 is `XL` (10 less than 50)
  - 90 is `XC` (10 less than 100)
  - 400 is `CD` (100 less than 500)
  - 900 is `CM` (100 less than 1000)
- Only powers of 10 (`I`, `X`, `C`, `M`) can be appended consecutively, up to at most 3 times. `V`, `L`, and `D` can never be appended consecutively.
...
### Solution - Digit-by-Digit Decomposition

#### Walkthrough

Process the integer one decimal digit at a time from least significant to most significant. At each step, extract the last digit with `num % 10` and convert it to its Roman representation based on its decimal place (ones, tens, hundreds, thousands). Prepend the result to the output string, then remove the processed digit with `num / 10`.

Each digit maps to Roman numerals by pattern:

- **1–3**: repeat the base symbol for that place (I, X, C, M) `digit` times.
- **4, 9**: direct lookup — these are the subtractive pairs (IV, XL, CD for 4; IX, XC, CM for 9).
- **5**: direct lookup — V, L, D.
- **6–8**: V/L/D symbol for that place, followed by 1–3 repetitions of the base symbol.

**Trace on `1994`:**

| digit | place | token | prepended result |
|-------|-------|-------|-----------------|
| 4 | 0 | IV    | "IV"      |
| 9 | 1 | XC    | "XCIV"    |
| 9 | 2 | CM    | "CMXCIV"  |
...
| Complexity | Value |
|------------|-------|
| Time       | $O(1)$ — at most 4 digits for inputs $\le 3999$ |
| Space      | $O(1)$ — output length is bounded by a constant |

#### Implementation Steps

1. Build a `Map<String, Integer>` with all 13 Roman tokens (7 single-character + 6 subtractive pairs).
2. In `intToRoman`, loop while `num > 0`: extract last digit via `num % 10`, convert it with `convertRomanLetters(digit, place)`, prepend to result, divide `num` by 10, increment `place`.
3. In `convertRomanLetters`, use a switch on `digit`:
   - Cases 4, 5, 9: direct map lookup for `digit * 10^place`.
   - Cases 6, 7, 8: recurse for 5 at same place, then recurse for `digit - 5` at same place.
   - Default (0–3): loop `digit` times appending `getRomanLetterFromMap(10^place)`.
4. `getRomanLetterFromMap` does a reverse lookup (value → key) over the map.

#### Code - Java

```java
class Solution {
    // All 13 valid Roman tokens: 7 single-character symbols + 6 subtractive pairs
    ...
    public String intToRoman(int num) {
        String result = "";
        // `place` tracks the current decimal position: 0 = ones, 1 = tens, 2 = hundreds, 3 = thousands
        int place = 0;

        while (num > 0) {
            int digit = num % 10;

            // Convert this digit at its decimal place and prepend (least significant first)
            result = convertRomanLetters(digit, place) + result;
    ...
    // Reverse lookup: find the Roman token whose value equals num
    private static String getRomanLetterFromMap(int num) {
    ...
    // Convert a single digit (0–9) at the given decimal place to its Roman representation
    private static String convertRomanLetters(int digit, int place) {
        // Absolute value of this digit (e.g. digit=9, place=1 → 90)
        int num = digit * Math.powExact(10, place);

        String result = "";

        switch (digit) {
            // Subtractive pairs and 5: direct map lookup covers IV/V/IX, XL/L/XC, CD/D/CM
            case 4:
            case 5:
            case 9:
                return getRomanLetterFromMap(num);
            // 6–8: V/L/D symbol for this place + 1–3 repetitions of the base symbol
            case 6:
            case 7:
            case 8:
                result += convertRomanLetters(5, place);
                result += convertRomanLetters(digit - 5, place);
                return result;
            // 0–3: repeat the base symbol (I/X/C/M) `digit` times
            default:
                for (int i = 0; i < digit; i++) {
                    result += getRomanLetterFromMap(Math.powExact(10, place));
                }
                return result;
        }
    }
...
## Roman to Integer {#lc-roman-to-integer}
...
### Solution - Enum Lookup with Subtraction Rule

#### Walkthrough

By reducing the enum to only the 7 single-character symbols, the subtraction pairs no longer need to be enumerated explicitly. Instead, the algorithm relies on the observation that a subtraction case always has a smaller-valued symbol immediately before a larger-valued symbol:

1. At each position `i`, look up the current character's value.
2. If a next character exists and its value is **greater** than the current value, **subtract** the current value (it is the smaller half of a subtraction pair).
3. Otherwise, **add** the current value.
4. The last character always falls into the add case (no next to compare).

This removes the switch statement entirely and makes the loop body uniform.

**Trace on `"MCMXCIV"`:**

| i | c | curr | next | next > curr? | op | result |
|---|---|------|------|--------------|----|--------|
...
| Space      | $O(1)$ — enum is a compile-time constant |

Compared to the two-character lookahead solution, this approach is simpler: the enum shrinks from 13 to 7 entries and the switch statement is replaced by a single conditional.

#### Implementation Steps

1. Define enum `Roman` with only the 7 single-character symbols and their values.
2. Add a `value(char c)` overload for convenience.
3. In `romanToInt`, iterate with index `i`:
   - If `i+1 < s.length()` and `value(s[i+1]) > value(s[i])`, subtract `value(s[i])`.
   - Otherwise, add `value(s[i])`.
   - Increment `i` by 1 each iteration (always single-character steps).
...
class Solution {
    private enum Roman {
        ...
        private Roman(String s, Integer v) {
            str = s;
            val = v;
        }
        ...
        public static int value(String s) {
            for (Roman roman : Roman.values()) {
                if (roman.str.equals(s)) {
                    return roman.val;
                }
            }
            throw new IllegalArgumentException(s + " does NOT exist in enum!");
        }
    }
    ...
    public int romanToInt(String s) {
        ...
        while (i < s.length()) {
            ...
            if ((i + 1) < s.length()) {
                ...
                if (Roman.value(nextC) > Roman.value(c)) {
                    result -= Roman.value(c);
                } else {
                    result += Roman.value(c);
                }
            } else {
...
### Alternative Solution - HashMap with Subtraction Rule

#### Walkthrough

Instead of an enum, a `HashMap<Character, Integer>` maps each of the 7 Roman symbols to its value. The subtraction logic is identical to the enum solution: compare the current character's value against the next character's value. This approach trades the compile-time enum definition for a runtime map initialized in the constructor or as a static field.

#### Analysis

| Complexity | Value |
|------------|-------|
| Time       | $O(n)$ — single pass; `HashMap.get()` is $O(1)$ |
| Space      | $O(1)$ — map holds exactly 7 fixed entries |

#### Implementation Steps

1. Initialize a `HashMap<Character, Integer>` with all 7 Roman symbol mappings.
2. Iterate with index `i` from 0 to `s.length() - 1`:
   - If `i+1 < s.length()` and `map.get(s[i+1]) > map.get(s[i])`, subtract `map.get(s[i])`.
   - Otherwise, add `map.get(s[i])`.
...
1. Use `index` to track current character position, starting at 0.
2. While `index` is within the first string's length, read character `c` at that position.
3. For each string, stop if `index` exceeds its length or the character doesn't match `c`.
4. If any mismatch is found, break; otherwise advance `index`.
5. Return `strs[0].substring(0, index)`.
...
class Solution {
    public String longestCommonPrefix(String[] strs) {
        ...
        while(index < strs[0].length()) {
            ...
            for(String str : strs) {
                //if the position already exceed the string length OR
                //if the char at the position doesn't match
...
## Longest Substring with At Most Two Distinct Characters {#lc-longest-substring-two-distinct}
...
- **Tags**: Adobe, Amazon, Facebook, Google, Microsoft
- **Techniques**: [Sliding Window](glossary-of-algorithm.html#glossary-sliding-window), [Hash Table](glossary-of-algorithm.html#glossary-hash-table), [Two Pointers](glossary-of-algorithm.html#glossary-two-pointer)

### Description

Given a string `s`, return the length of the longest substring that contains at most two distinct characters.

**Note**: This is a premium LeetCode problem that requires a subscription to access.
...
This problem uses a **sliding window technique** combined with a **frequency map** to find the longest valid substring. The key insight is maintaining a window that always contains at most 2 distinct characters.

**Core Concept - Window Validity:**

- A window `[left, right]` is valid when it contains at most 2 distinct characters
- We use a frequency map to track the count of each character in the current window
- The number of distinct characters is simply `len(freq)`
- When `len(freq) > 2`, the window becomes invalid and must shrink
...
3. **Expand the window**: Move `right` pointer and add characters
4. **Validate the window**: Check if `len(freq) <= 2`
5. **Shrink when invalid**: Move `left` pointer forward when more than 2 distinct characters exist
6. **Record maximum**: Track the largest valid window size encountered

**Why Frequency Map (Not Just a Set)?**

A common mistake is using a set to track distinct characters and trying to optimize by "skipping consecutive duplicates." This breaks the sliding window invariant because:

- After removing all consecutive occurrences of a character, the set doesn't reflect what's actually left in the window
- The frequency map ensures we always know exactly which characters are in `s[left:right+1]`
- We must shrink **one character at a time** to maintain correctness

**Visual Example with `s = "eceba"`:**

```
Initial: left = 0, right = 0, freq = {}, result = 0
...
Step 2: right = 1, char = 'c'
  Window: [ec]
  freq = {e:1, c:1}, distinct = 2
  windowLen = 2
  result = 2

Step 3: right = 2, char = 'e'
  Window: [ece]
  freq = {e:2, c:1}, distinct = 2
  windowLen = 3
  result = 3

Step 4: right = 3, char = 'b' (3 distinct!)
  Before shrink: Window [eceb], freq = {e:2, c:1, b:1}, distinct = 3 ✗

  Shrink iteration 1:
    Remove s[0]='e', freq[e]-- → freq = {e:1, c:1, b:1}, still 3 distinct
    left = 1

  Shrink iteration 2:
    Remove s[1]='c', freq[c]-- → freq[c]=0, delete 'c' → freq = {e:1, b:1}
    left = 2

  Now: Window [eb], freq = {e:1, b:1}, distinct = 2 ✓
  windowLen = 2
  result = 3 (unchanged)

Step 5: right = 4, char = 'a' (3 distinct!)
  Before shrink: Window [eba], freq = {e:1, b:1, a:1}, distinct = 3 ✗

  Shrink iteration:
    Remove s[2]='e', freq[e]-- → freq[e]=0, delete 'e' → freq = {b:1, a:1}
    left = 3

  Now: Window [ba], freq = {b:1, a:1}, distinct = 2 ✓
  windowLen = 2
  result = 3 (unchanged)
...
**Example with `s = "ccaabbb"`:**

```
Step 1-2: Window "cc", freq = {c:2}, len = 2
Step 3-4: Window "ccaa", freq = {c:2, a:2}, len = 4
Step 5: char = 'b' (3 distinct!)
  Shrink: Remove 'c' twice → Window "aab", freq = {a:2, b:1}, len = 3
Step 6-7: Window "aabbb", freq = {a:2, b:3}, len = 5 ✓
...
1. The window `[left, right]` always contains at most 2 distinct characters (after shrinking)
2. We shrink **one character at a time** to maintain the frequency map invariant
3. `len(freq)` gives us the number of distinct characters efficiently
4. We track the maximum window size seen throughout the process
...
- **Time Complexity**: O(n) where n is the length of string `s`
  ...
  - Each character is added to the window exactly once: O(n)
  - Each character is removed from the window at most once: O(n)
  - The inner shrinking loop amortizes to O(1) per right pointer iteration
  - Total: O(n)

- **Space Complexity**: O(1)
  - Frequency map stores at most 3 entries at any time (temporarily when we detect 3 distinct characters before shrinking)
  - Since the constraint is "at most 2 distinct characters," the map is bounded by a constant
...
1. **Initialize data structures:**
   - Create frequency map `freq` (map[byte]int in Go, HashMap<Character, Integer> in Java)
   - Initialize `left = 0` as the left pointer
   - Initialize `result = 0` to store the maximum window length

2. **Iterate with right pointer** from 0 to `s.length() - 1`:
   - This pointer expands the window to the right

3. **Add character to window:**
   - Get character at position `right`: `s[right]`
   - Increment its frequency: `freq[s[right]]++`

4. **Check window validity:**
   - Count distinct characters: `len(freq)` (Go) or `freq.size()` (Java)
   - If more than 2 distinct characters, window is invalid

5. **Shrink window while invalid:**
   - While `len(freq) > 2`:
     - Get character at position `left`: `s[left]`
     - Decrement its frequency: `freq[s[left]]--`
     - If frequency becomes 0, remove it from map: `delete(freq, s[left])`
     - Move left pointer forward: `left++`

6. **Update result:**
   - After ensuring window is valid, update result: `result = max(result, right - left + 1)`

7. **Return result** after completing the loop

#### Code - Go

```go
func lengthOfLongestSubstringTwoDistinct(s string) int {
    freq := make(map[byte]int)  // Track character frequencies
    l := 0
    result := 0
...
class Solution {
    public int lengthOfLongestSubstringTwoDistinct(String s) {
        ...
        for (int r = 0; r < s.length(); r++) {
            ...
            // Shrink the window when more than 2 distinct characters
            if (freqMap.size() > 2) {
                char lChar = s.charAt(l);
                int freq = freqMap.getOrDefault(lChar, 0);
                freqMap.put(lChar, --freq);
...
## Largest Number {#lc-largest-number}
...
- **Tags**: Top 100 Liked
- **Techniques**: [Sorting](#sorting-algorithms)
...
The key insight is defining the right ordering between any two numbers. When deciding whether number `a` should come before number `b`, compare the two concatenations `"ab"` vs `"ba"` as strings. Whichever is lexicographically larger determines the order.

**Why concatenation comparison works:**

Consider `a = 9` and `b = 34`:
- `"9" + "34"` = `"934"` = 934
- `"34" + "9"` = `"349"` = 349

Since 934 > 349, 9 should come first → result: `"934"`.

This comparator defines a total ordering across all numbers and can be directly used with `Arrays.sort`.

**Edge case — all zeros:**

If all numbers are `0`, the array sorts to all zeros (e.g., `[0, 0, 0]`). After joining we get `"000"`, which should be returned as `"0"`. Check if the first element after sorting is `"0"` to handle this.

**Step-by-step for `[3, 30, 34, 5, 9]`:**

1. Convert to strings: `["3", "30", "34", "5", "9"]`
2. Sort with custom comparator (compare `b+a` vs `a+b` for descending order):
...
- **Time Complexity**: O(n log n · k) where k is the average number of digits (string comparison per pair is O(k))
- **Space Complexity**: O(n) for the string array

#### Implementation Steps

1. Box `int[]` into `Integer[]` so `Arrays.sort` can accept a `Comparator`.
2. Scan for the all-zeros edge case during the boxing pass; return `"0"` immediately if true.
3. Implement `DigitComparator`: for two integers `n1` and `n2`, convert to strings `s1`/`s2` and return `(s2 + s1).compareTo(s1 + s2)` (negative means n1 goes first). Use string comparison — **not** `Double.valueOf` — to avoid floating-point precision loss on 18–20-digit concatenations.
4. Sort the `Integer[]` with `DigitComparator`, then join and return.

**Why string compareTo is safe here:** `s1+s2` and `s2+s1` always have the same length (`len(s1) + len(s2)`), so lexicographic order equals numeric order — no precision issues regardless of input size.
...
public String largestNumber(int[] nums) {
    ...
    Arrays.sort(array, new DigitComparator());
...
### Solution 2 - Tiled Key Sort (No Comparator)

#### Walkthrough

Instead of defining how to compare two numbers, assign each number an explicit **sort key** so that Java's natural `String` ordering produces the correct result — no `Comparator` needed.

**The tiled key:** Tile each number's string representation to exactly 10 characters (the maximum width of `10^9`). Numbers with higher tile values should appear earlier, so sort ascending and reverse.

**Why tiling is equivalent to concatenation comparison:**

Both `s1+s2` vs `s2+s1` and their tile values encode the same information. Tiling to a uniform length converts the comparison into a standard fixed-width string comparison.

Example for `[3, 30, 34, 5, 9]`:

| Number | Tile key (10 chars) |
|--------|---------------------|
| 9 | `9999999999` |
| 5 | `5555555555` |
| 34 | `3434343434` |
...
Natural ascending order: `30... < 33... < 34... < 55... < 99...`

Reversed (descending): `9, 5, 34, 3, 30` → `"9534330"` ✓

**Encoding trick:** Concatenate each 10-char tile key with the original string into a single entry. `Arrays.sort` on the entries uses natural order (no `Comparator`). Iterate in reverse to extract originals in descending key order.

#### Analysis

- **Time Complexity**: O(n log n · k) — same as Solution 1
- **Space Complexity**: O(n)

#### Implementation Steps

1. Convert each integer to its string, then tile it to 10 characters to form a sort key.
2. Encode each entry as `tileKey + originalStr` (tile key occupies fixed first 10 chars).
3. Call `Arrays.sort` with no comparator — natural ascending string order.
4. Iterate entries in reverse (descending key order), extracting the substring after position 10.

#### Code - Java

```java
public String largestNumber(int[] nums) {
    String[] entries = new String[nums.length];
    boolean isAll0s = true;
    ...
    for (int i = 0; i < nums.length; i++) {
        ...
        entries[i] = key.substring(0, 10) + s;
    }
    ...
    Arrays.sort(entries); // natural ascending order — no Comparator

    StringBuilder sb = new StringBuilder();
    for (int i = entries.length - 1; i >= 0; i--) {
        sb.append(entries[i].substring(10)); // strip the 10-char key prefix
    }
...
| | Solution 1 (Custom Comparator) | Solution 2 (Tiled Key Sort) |
|-|---|----|
| **Comparator** | Yes — `DigitComparator` class | None |
| **Sort order** | Descending directly | Ascending + reverse iteration |
| **Key insight** | Define pairwise order via `(s2+s1).compareTo(s1+s2)` | Create per-element key; rely on natural order |
| **Time** | O(n log n · k) | O(n log n · k) |
| **Space** | O(n) | O(n) |
| **Pitfall** | Must use `String.compareTo`, not `Double.valueOf` | Encoding key must be fixed-width (10 chars) |
...
## Longest Substring with At Most K Distinct Characters {#lc-longest-substring-k-distinct}
...
**Note**: This is a premium LeetCode problem that requires a subscription to access. This is the generalized version of [Longest Substring with At Most Two Distinct Characters](#lc-longest-substring-two-distinct) (LeetCode 159), where the fixed limit of 2 is replaced by a variable `k`.
...
This is the generalized form of LeetCode 159. The only change is replacing the hard-coded constant `2` with the parameter `k`. All the same reasoning applies: maintain a sliding window `[left, right]` where the frequency map has at most `k` entries. When `freqMap.size() > k`, shrink the window from the left one character at a time.

**Edge Case - k = 0:**

When `k = 0`, no characters can be in the window, so the result is always `0`. The algorithm handles this naturally: after adding any character, `freqMap.size() > 0` triggers shrinking until the left pointer catches up to right, keeping the window empty.

#### Analysis

- **Time Complexity**: O(n) — each character is added and removed from the window at most once
- **Space Complexity**: O(k) — the frequency map holds at most k+1 entries at any time

#### Implementation Steps

1. **Initialize data structures:** frequency map, `left = 0`, `result = 0`
2. **Iterate with right pointer** from 0 to `s.length() - 1`
3. **Add character:** increment `freqMap[s[right]]`
4. **Shrink while invalid:** while `freqMap.size() > k`, decrement `freqMap[s[left]]`, remove if zero, advance `left`
5. **Update result:** `result = max(result, right - left + 1)`
6. **Return result**
...
class Solution {
    public int lengthOfLongestSubstringKDistinct(String s, int k) {
        ...
        for (int r = 0; r < s.length(); r++) {
            ...
            // Shrink the window when more than k distinct characters
            if (freqMap.size() > k) {
                char lChar = s.charAt(l);
                int freq = freqMap.getOrDefault(lChar, 0);
                freqMap.put(lChar, --freq);
...
## Find the Length of the Longest Common Prefix {#lc-find-length-longest-common-prefix}
...
- **Tags**:
- **Techniques**: [Trie](glossary-of-algorithm.html#glossary-trie), [Hash Table](glossary-of-algorithm.html#glossary-hash-table), String
...
### Solution - Hash Set (Integer Prefixes)

#### Walkthrough

For every number in `arr1`, repeatedly divide by 10 to strip the last digit, adding each intermediate value to a `HashSet<Integer>`. This builds a set of all integer prefixes without any string conversion.

Then for every number in `arr2`, do the same stripping from the right. The first match found is the longest possible prefix for that number (since we go from more digits to fewer), so we can break immediately and update the result.

#### Analysis

- **Time Complexity**: O((n + m) · D), where `n` and `m` are the lengths of `arr1` and `arr2`, and `D` is the max number of digits ($\le$ 9 for values up to $10^8$). Each number generates at most D integer prefixes via division. Since D is a small constant, this is effectively O(n + m).
- **Space Complexity**: O(n · D) for the prefix set — up to D integer entries per `arr1` number.

#### Implementation Steps

1. For each number in `arr1`, while `num > 0`: add `num` to the set, then `num /= 10`.
2. For each number in `arr2`, while `num > 0`: if `num` is in the set, update result with the digit count of `num` and break; otherwise `num /= 10`.
3. Return the result.
...
class Solution {
    public int longestCommonPrefix(int[] arr1, int[] arr2) {
        Set<Integer> commonPrefixSet = new HashSet<>();

        for(int num : arr1) {
            while(num > 0) {
                commonPrefixSet.add(num);
                num /= 10;
            }
        }
        ...
        for(int num : arr2) {
            while(num > 0) {
                if(commonPrefixSet.contains(num)) {
...
func longestCommonPrefix(arr1 []int, arr2 []int) int {
    ...
    for _, num := range arr2 {
        for num > 0 {
            if commonPrefixSet.Has(num) {
                result = max(result, len(strconv.Itoa(num)))
            }
            num /= 10
        }
    }
