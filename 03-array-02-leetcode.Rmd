...
- **Tags**:
- **Techniques**: [Bucket Sort](#sorting-bucket-sort), [Sorting](glossary-of-algorithm.html#glossary-sorting)
...
Explanation: The sorted form of the array is [1,3,6,9], either
             (3,6) or (6,9) are successive elements with a maximum
             difference of 3.
```
...
Input: nums = [10]
Output: 0
Explanation: The array contains fewer than 2 elements, so return 0.
```
...
### Solution - Bucket Sort (Pigeonhole Principle)

#### Walkthrough

The linear-time constraint rules out comparison-based sorting (O(n log n)). The key insight comes from the **pigeonhole principle**:

**Average gap reasoning:**

For `n` elements spanning range `[min, max]`, the average gap between consecutive sorted elements is:

$$\text{avgGap} = \frac{\text{max} - \text{min}}{n - 1}$$

If we divide the range into `n-1` buckets each of width $\le$ `avgGap`, then by the pigeonhole principle at least one bucket must be **empty** — we have `n` elements but `n-1` buckets, with `min` and `max` locked at the endpoints.

**Critical observation:** Because at least one bucket is empty, the maximum gap **cannot come from within a single bucket** (all values in a bucket differ by at most `bucketWidth` $\le$ `avgGap`). The maximum gap **must span across at least one bucket boundary**.

Therefore: we only need to track each bucket's local `{min, max}` — not its full contents — and scan gaps between adjacent non-empty buckets.

**Example walkthrough (`[3,6,9,1]`):**

```
min=1, max=9, n=4
bucketWidth = max(1, 8/3) = 2
bucketCount = 8/2 + 1 = 5

Bucket 0 [1,2]: {1,1}   ← contains 1
Bucket 1 [3,4]: {3,3}   ← contains 3
Bucket 2 [5,6]: {6,6}   ← contains 6
Bucket 3 [7,8]: empty   ← pigeonhole guarantee
Bucket 4 [9,10]: {9,9}  ← contains 9

Scan (tracking prevMax across non-empty buckets):
  prevMax = 1 (bucket 0)
  gap 0→1: 3 - 1 = 2,  prevMax = 3
  gap 1→2: 6 - 3 = 3,  prevMax = 6
  bucket 3: skip (empty)
  gap 2→4: 9 - 6 = 3,  prevMax = 9
...
- **Time Complexity**: O(n)
  - Two passes to find `min`/`max`: O(n)
  - One pass to fill buckets: O(n)
  - One pass to scan bucket gaps: O(n) (bucketCount is bounded by n)
  - No comparison-based sort — meets the linear-time requirement

- **Space Complexity**: O(n)
  - `bucketCount = (max - min) / bucketWidth + 1`. Since `bucketWidth` $\ge$ `(max-min)/(n-1)`, we have `bucketCount` $\le$ `n-1+1 = n`
  - Each bucket stores only 2 integers `{min, max}`

#### Implementation Steps

**Edge cases (pre-check):**

1. If `nums.length <= 1`, return `0`
2. If `nums.length == 2`, return `|nums[0] - nums[1]|`
3. After finding `min`/`max`: if `min == max` (all elements equal), return `0`

**Bucket setup:**

4. Compute `bucketWidth = max(1, (max - min) / (n - 1))` — floor division, minimum 1 to avoid divide-by-zero
5. Compute `bucketCount = (max - min) / bucketWidth + 1`
6. Initialize `bucketMinMax[bucketCount][2]` with `{MAX_VALUE, MIN_VALUE}` sentinels per bucket

**Fill buckets:**

7. For each `num`, compute `bucketIdx = (num - min) / bucketWidth`
8. Update `bucketMinMax[bucketIdx]` with the local min and max

**Scan gaps:**

9. Initialize `prevMax = bucketMinMax[0][1]` (max of first bucket)
10. Iterate `i` from 1 to `bucketCount - 1`:
    - If bucket `i` is empty (`bucketMinMax[i][0] == MAX_VALUE`): skip
    - Otherwise: `gap = bucketMinMax[i][0] - prevMax`, update `maxGap`, update `prevMax = bucketMinMax[i][1]`
11. Return `maxGap`
...
public int maximumGap(int[] nums) {
    ...
    int[][] bucketMinMax = new int[bucketCount][2];
    for (int i = 0; i < bucketCount; i++) {
        bucketMinMax[i] = new int[]{Integer.MAX_VALUE, Integer.MIN_VALUE};
    }

    for (int num : nums) {
        int bucketIdx = (num - min) / bucketWidth;
        bucketMinMax[bucketIdx][0] = Math.min(bucketMinMax[bucketIdx][0], num);
        bucketMinMax[bucketIdx][1] = Math.max(bucketMinMax[bucketIdx][1], num);
    }

    int maxGap = 0;
    int prevMax = bucketMinMax[0][1];
    for (int i = 1; i < bucketCount; i++) {
        if (bucketMinMax[i][0] == Integer.MAX_VALUE) {
            continue;
        }
        maxGap = Math.max(maxGap, bucketMinMax[i][0] - prevMax);
        prevMax = bucketMinMax[i][1];
    }
...
### Solution - Two Pointers (Lazy-Initialized Slow Pointer)

#### Walkthrough

Use a **lazy-initialized slow pointer** `l` (set to `-1` until the first zero is found) and a fast pointer `r` that scans the array. Once `l` points to the first zero, every subsequent non-zero element at `r` is swapped with `nums[l]`, then `l` advances. Because all positions between `l` and `r` are zeros (they were never swapped), `l++` always lands on the next zero — preserving the invariant.

**Step-by-step for `nums = [0,1,0,3,12]`:**

```
r=0: nums[0]=0, l<0   → l=0
r=1: nums[1]=1, l=0   → swap(0,1) → [1,0,0,3,12], l=1
r=2: nums[2]=0, l≥0   → skip (l already set)
r=3: nums[3]=3, l=1   → swap(1,3) → [1,3,0,0,12], l=2
r=4: nums[4]=12, l=2  → swap(2,4) → [1,3,12,0,0], l=3
Result: [1,3,12,0,0]
```

**Why `l++` always lands on a zero after a swap:**

Before a swap, `nums[l]` is a zero and all positions `l+1 ... r-1` were skipped (they were zeros too — the `l<0` guard prevented re-assigning `l`). After swapping `nums[l]` with the non-zero at `r`, `l+1` is guaranteed to hold a zero.
...
1. Initialize `l = -1` (sentinel: no zero found yet).
2. Iterate `r` from 0 to `nums.length - 1`:
   1. If `nums[r] == 0` and `l < 0`: set `l = r` (mark first zero position).
   2. If `nums[r] != 0` and `l >= 0`: swap `nums[l]` with `nums[r]`, then `l++`.
3. After the loop, all non-zero elements are compacted to the front; zeros fill the tail.
...
#### Alternative: Eager Slow Pointer

Initializing `insertPos = 0` instead of `-1` simplifies the logic at the cost of redundant self-swaps when `insertPos == r` (before any zero is encountered).
...
| | Lazy (`l = -1`) | Eager (`insertPos = 0`) |
|---|---|---|
| Self-swaps | None | Yes, until first zero |
| Logic | Two `if` branches per case | Single `if (nums[i] != 0)` |
| Swap count | Minimal | Slightly more |
...
## Find K Closest Elements {#lc-find-k-closest-elements}
...
- **Tags**:
- **Techniques**: [Binary Search](#binary-search-strategy), [Two Pointers](glossary-of-algorithm.html#glossary-two-pointer), [Heap](glossary-of-algorithm.html#glossary-heap), [Sorting](glossary-of-algorithm.html#glossary-sorting), Custom Comparator
...
### Solution 1 - Max Heap (Expand from Insertion Point)

#### Walkthrough

Find the rightmost index where $arr[index] \le x$ — this is the closest element to `x` from the left. Then expand outward using two pointers `l` (leftward) and `r` (rightward), feeding candidates into a **max-heap ordered by distance from x**.

The heap always evicts the farthest element when it grows beyond `k`, so after scanning up to `k` elements from each side, exactly the `k` closest remain. The tie-breaking rule (prefer smaller value on equal distance) is enforced by the comparator: when distances are equal, the larger element gets higher priority in the heap and is evicted first.

#### Analysis

- **Time Complexity**: O(n + k log k) — O(n) linear scan to find insertion point, O(k log k) for heap operations (at most 2k insertions each costing O(log k)), O(k log k) for final sort
- **Space Complexity**: O(k) for the heap

#### Implementation Steps

1. Walk `index` forward until `arr[index + 1] > x`, marking the last element $\le$ x.
2. Set left pointer `l` to `index` (elements $\le$ x) and right pointer `r` to `index + 1` (elements $>$ x).
3. Add up to `k` elements from the left into the max-heap; evict the farthest if size exceeds `k`.
4. Add up to `k` elements from the right into the max-heap; evict the farthest if size exceeds `k`.
5. Drain the heap into a list and sort ascending (heap order is by distance, not value).
...
class Solution {
    private class DistanceComparator implements Comparator<Integer> {
        ...
        // Max-heap by distance: the element farthest from x sits at the top and is polled first.
        // Tie-break: the larger value has higher priority so it gets evicted first,
        // keeping the smaller value (per the problem's "prefer smaller" rule).
        public int compare(Integer n1, Integer n2) {
            int d1 = getDistance(n1, x);
            int d2 = getDistance(n2, x);
            if (d1 > d2) {
                return -1;  // n1 is farther → higher priority → polled first
            } else if (d1 == d2) {
                return n2 - n1;  // equal distance: larger value → higher priority → evicted first
            } else {
                return 1;
            }
        }

        private int getDistance(int n, int x) {
            return Math.abs(x - n);
        }
    }

    public List<Integer> findClosestElements(int[] arr, int k, int x) {
        // Find the rightmost index where arr[index] <= x (the closest element from the left).
        int index = 0;
        ...
        // l scans leftward (elements <= x), r scans rightward (elements > x).
        int l = index, r = index + 1;
        PriorityQueue<Integer> queue = new PriorityQueue<>(new DistanceComparator(x));

        // Collect up to k candidates from the left; evict the farthest if size exceeds k.
        for (int i = 0; i < k && l >= 0; i++, l--) {
            queue.add(arr[l]);
            if (queue.size() > k) {
                queue.poll();
            }
        ...
        // Collect up to k candidates from the right; evict the farthest if size exceeds k.
        for (int i = 0; i < k && r < arr.length; i++, r++) {
            queue.add(arr[r]);
            if (queue.size() > k) {
                queue.poll();
            }
        ...
        // Drain heap and sort: heap order is by distance, but output must be ascending by value.
        List<Integer> result = new ArrayList<>();
        for (int i = 0; i < k; i++) {
...
### Solution 2 - Binary Search on Window Start

#### Walkthrough

**Key insight:** In a sorted array, the k closest elements always form a **contiguous window**. So instead of picking elements one by one, we binary search for the optimal window start index.

The search space for the window start is `[0, n - k]` (the last valid start that leaves room for k elements). For any candidate start `mid`, we look at the two elements just outside the candidate window: `arr[mid]` (left edge) and `arr[mid + k]` (right edge, one past the window). If `x - arr[mid] > arr[mid + k] - x`, the left edge is farther away than the right edge, so the window should slide right (`left = mid + 1`). Otherwise, the left edge is at least as close — keep it and shrink from the right (`right = mid`).

When the loop ends, `left` is the optimal window start. Because `arr` is already sorted, slicing `arr[left..left+k-1]` gives a sorted result directly — no extra sort needed.

#### Analysis

- **Time Complexity**: O(log(n - k) + k) — O(log(n-k)) for binary search, O(k) to build the result list
- **Space Complexity**: O(1) auxiliary (excluding output list)

#### Implementation Steps

1. Set left to 0 and right to `arr.length - k` (search space for window start).
2. While left $<$ right:
   - Compute `mid` as `left + (right - left) / 2`.
   - If `x - arr[mid] > arr[mid + k] - x`: left edge is farther → slide window right (left becomes `mid + 1`).
   - Else: left edge is at least as close → keep it, shrink from right (right becomes `mid`).
3. Collect `arr[left..left+k-1]` into the result list (already sorted).
...
public List<Integer> findClosestElements(int[] arr, int k, int x) {
    int left = 0;
    int right = arr.length - k;  // last valid window start index

    // Binary search for the leftmost position of the optimal k-element window.
    while (left < right) {
        int mid = left + (right - left) / 2;

        // Compare the two elements just outside the candidate window [mid, mid+k-1]:
        //   arr[mid]   is the left edge (distance from x: x - arr[mid])
        //   arr[mid+k] is one past the right edge (distance from x: arr[mid+k] - x)
        if (x - arr[mid] > arr[mid + k] - x) {
            // Left edge is strictly farther → slide the window right.
            left = mid + 1;
        } else {
            // Left edge is at least as close (tie goes to smaller value per problem rule)
            // → keep arr[mid], shrink the search from the right.
            right = mid;
        }
    }

    // left is now the start of the optimal window; collect k consecutive elements.
    List<Integer> result = new ArrayList<>();
    for (int i = left; i < left + k; i++) {
        result.add(arr[i]);
    }

    // No sort needed: the window is already a contiguous slice of the sorted input.
    return result;
}
```
