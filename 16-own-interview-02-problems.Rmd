...
**Expected Answer**: A `<defunct>` process is a **Zombie Process**. It means the child process has actually terminated, but its parent process (our CLI tool) hasn't acknowledged it by reading its exit status (via the `wait()` system call). The zombie is kept in the OS process table so the parent can eventually read the exit code. To get rid of it, the parent process must call `wait()` (or handle the `SIGCHLD` signal), or the parent process itself must be killed (at which point the OS init system, like `systemd`, will inherit the zombie and reap it).
