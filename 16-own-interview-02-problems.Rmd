...
| 2. Group Role Inheritance | Medium | Data modeling, set union operations |
| 3. The Global Admin | Easy | Short-circuiting, code efficiency |
...
```java
// Inside the evaluation loop for a feature:
...
#### Follow-Up 2: Group Role Inheritance

**Scenario**: We introduce a `Group` concept (e.g., "Beta Testers", "US Team"). A User can belong to multiple groups, and each Group has its own list of Roles.

**Question**: "How do you adjust the algorithm so that a user's final effective roles are the union of their direct roles and the roles of all groups they belong to?"

**Expected Answer**: 
The candidate should realize this aggregation must happen *before* the evaluation loop. They should create a single `HashSet` representing the "effective roles". They initialize the set with the user's direct roles, then iterate through the user's groups, looking up each group's roles, and adding those roles to the same `HashSet`. Because it's a Set, duplicates are automatically handled, perfectly mirroring a mathematical Union operation.

```java
Set<String> effectiveRoles = new HashSet<>(user.roles);

// Union operation: Add all roles from all groups the user belongs to
for (Group group : user.groups) {
    effectiveRoles.addAll(group.roles);
}

// Now proceed with the evaluation loop using `effectiveRoles` instead of user.roles
```
...
// Now proceed with the evaluation loop using `effectiveRoles`
```

#### Follow-Up 3: The Global Admin (Short-circuiting)

**Scenario**: We introduce a `"super_admin"` role. If a user has this role, they instantly have access to every feature, regardless of the individual feature configurations.

**Question**: "Where do you place this logic to make the function as efficient as possible?"

**Expected Answer**: 
At the very top of the function. The candidate should check if the user has `"super_admin"`. If so, they shouldn't even loop through the individual feature rules. They can just do a fast loop returning a map with all features set to `true` (Early Return).

```java
public Map<String, Boolean> evaluateFeatures(User user, List<FeatureConfig> features) {
    Map<String, Boolean> results = new HashMap<>();
    
    // Short-circuit for Global Admin
    if (user.roles.contains("super_admin")) {
        for (FeatureConfig feature : features) {
            results.put(feature.featureName, true);
        }
        return results; // Return immediately, saving compute
    }
    
    // ... rest of the standard evaluation logic ...
}
```
...
func EvaluateFeatures(user User, features []FeatureConfig) map[string]bool {
	...
	for _, role := range user.Roles {
		if role == "super_admin" {
			for _, feature := range features {
				results[feature.FeatureName] = true
			}
			return results // Return immediately, saving compute
		}
	}
...
**Scenario**: This `evaluateFeatures` function is being called on every single API request. We now have 10,000 features. The CPU load is getting too high.
...
**Expected Answer**: 
Caching! Since feature evaluations only depend on the user's Roles, we can evaluate it **once upon login**. We can then embed the resulting permissions map directly into the user's Session Storage, a distributed cache like Redis, or encrypt it inside their JWT (JSON Web Token) to be passed with every request.
