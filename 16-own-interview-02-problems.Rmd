...
> [!TIP]
> **Interviewer Tip (System Crash Analysis)**: Ask the candidate: *"Do zombie processes consume CPU and RAM? What happens if our CLI tool leaks thousands of them?"* A strong systems engineer will know that zombies consume almost **zero** CPU and memory (their resources have already been freed). However, they consume a slot in the **Process Table**. Since Linux has a strict maximum PID limit (often 32,768), leaking too many zombies will exhaust the process table. Once full, the OS cannot spawn *any* new processes, effectively paralyzing the server with `Resource temporarily unavailable` (or `fork: retry: No child processes`) errors until the parent is killed.
