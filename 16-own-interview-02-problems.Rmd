...
[
  {
    ...
    "input": ["testA", "testB", "testC", "testD", "target"],
    "polluter": "testC",
    "output": 2
  },
  {
    "description": "Polluter is the very first test",
    "input": ["testA", "testB", "testC", "target"],
    "polluter": "testA",
    "output": 0
  },
  {
    "description": "Polluter is right before the target",
    "input": ["testA", "testB", "target"],
    "polluter": "testB",
    "output": 1
  },
  {
  ...
  },
  {
    ...
    "input": ["t1", "t2", "t3", "t4", "t5", "t6", "t7", "t8", "target"],
    "polluter": "t7",
    "output": 6
  }
]
...
```java
public int findPollutingTest(List<String> tests, Function<List<String>, Boolean> runTests) {
    String targetTest = tests.get(tests.size() - 1);
    int left = 0;
    ...
    while (left < right) {
        ...
        List<String> testChunk = new ArrayList<>(tests.subList(left, mid + 1));
...
```go
func FindPollutingTest(tests []string, runTests func([]string) bool) int {
	...
	for left < right {
		...
		testChunk := make([]string, 0, mid-left+2)
...
```java
public int findPollutingTestRecursive(List<String> tests, Function<List<String>, Boolean> runTests) {
    String targetTest = tests.get(tests.size() - 1);
    return search(tests, targetTest, 0, tests.size() - 2, runTests);
}

private int search(List<String> tests, String targetTest, int left, int right, Function<List<String>, Boolean> runTests) {
    if (left >= right) {
        return left;
    }
...
```go
func FindPollutingTestRecursive(tests []string, runTests func([]string) bool) int {
	targetTest := tests[len(tests)-1]
...
```java
public List<Integer> findAllPollutingTests(List<String> tests, Function<List<String>, Boolean> runTests) {
    String targetTest = tests.get(tests.size() - 1);
...
private void searchAll(List<String> tests, String targetTest, int left, int right, Function<List<String>, Boolean> runTests, List<Integer> results) {
    if (left > right) {
        return;
    }
    
    List<String> testChunk = new ArrayList<>(tests.subList(left, right + 1));
    testChunk.add(targetTest);
...
```go
func FindAllPollutingTests(tests []string, runTests func([]string) bool) []int {
	...
	search = func(left, right int) {
		...
		chunk := append(append([]string{}, tests[left:right+1]...), target)
...
class TestCase {
    String name;
    TestCase interactingWith; // Null if no interaction
...
type TestCase struct {
	Name            string
	InteractingWith *TestCase // Nil if no interaction
}
...
```java
public int findFlakyTest(List<String> tests, Function<List<String>, Boolean> runTests, int retryLimit) {
    String target = tests.get(tests.size() - 1);
    int left = 0;
    int right = tests.size() - 2;

    while (left < right) {
        int mid = left + (right - left) / 2;
        List<String> chunk = new ArrayList<>(tests.subList(left, mid + 1));
        chunk.add(target);
...
```go
func FindFlakyTest(tests []string, runTests func([]string) bool, retryLimit int) int {
	target := tests[len(tests)-1]
	left := 0
	right := len(tests) - 2
	...
	for left < right {
		...
		chunk := make([]string, 0, mid-left+2)
