...
- Return combinations as lists of **indices** (not weight values)
...
3. **Why must we copy `current` when adding to `result`?**
   ...
   - Without copying, all entries in `result` would reference the same (eventually empty) list
...
2. **Work per recursive call**: $O(k)$ for copying `current` list when adding to result
...
2. **Current combination list**: $O(T/W_{min})$ - maximum length of a single combination
3. **Output list**: Not counted as auxiliary space (required by problem)
...
- Create `result` list to store all valid combinations
- Initialize backtracking with empty `current` list, starting index 0, and full capacity
...
**Relationship to Similar Problems**:
This is a variant of [Coin Change](#coin-change) (LeetCode 322). Both problems use unbounded knapsack DP to find the minimum number of items (coins/plans) needed to reach a target sum. The core algorithm is identical, with different variable names (planSizes vs coins, targetBandwidth vs amount).
...
This problem is a **classic unbounded knapsack optimization** problem, identical in structure to [Coin Change](#coin-change) (LeetCode 322). The key insight is that we're not finding all combinations (which would require backtracking), but rather finding the **minimum count** of plans needed, which is perfect for dynamic programming.
