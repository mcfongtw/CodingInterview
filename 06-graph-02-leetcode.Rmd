...
## Alien Dictionary {#lc-alien-dict-order}
...
- **Techniques**: [Topological Sort](glossary-of-algorithm.html#glossary-topological-sort), [Directed Acyclic Graph](glossary-of-algorithm.html#glossary-dag), [Breadth First Search](glossary-of-algorithm.html#glossary-bfs), [Hash Table](glossary-of-algorithm.html#glossary-hash-table), Graph
...
- The input is guaranteed to be from a valid alien language dictionary
...
1. **Build the dependency graph** by comparing adjacent words
2. **Detect invalid inputs** (prefix violations and cycles)
3. **Apply DFS-based topological sort** to find the ordering
...
1. **Prefix Violation:** If a longer word appears before its prefix (e.g., `["abc", "ab"]`), the input is invalid → return `""`.

2. **Cycle Detection:** If the derived graph contains a cycle (e.g., `a → b → c → a`), no valid ordering exists → return `""`.
...
1. **Initialize graph with all unique characters:**
   - Create a `HashMap<Character, Set<Character>>` for the adjacency list
   - Iterate through all words and add each character as a node (even if it has no edges)

2. **Build edges by comparing adjacent words:**
   - For each pair of consecutive words `(word1, word2)`:
     - Check for prefix violation: if `word1.length() > word2.length()` and `word1.startsWith(word2)`, return `""`
     - Find the first position where characters differ
     - Add edge `word1[pos] → word2[pos]`
     - Break after finding the first difference (only the first difference matters)

3. **Perform DFS with cycle detection:**
   - Use an `int[] state` array with three states: WHITE (0), GRAY (1), BLACK (2)
   - For each unvisited character (WHITE), start DFS
   - In DFS:
...
## Network Delay Time {#lc-network-delay-time}
...
- **Tags**: NeetCode 150
- **Techniques**: [Dijkstra's Algorithm](#dijkstra-algorithm), [Bellman-Ford Algorithm](#bellman-ford-algorithm), Weighted Graph, Shortest Path
