...
- **Techniques**: [Depth First Search](glossary-of-algorithm.html#glossary-dfs), [Dynamic Programming](#dynamic-programming), [Two Pointers](glossary-of-algorithm.html#glossary-two-pointer)
...
### Depth First Traversal {#glossary-dfs}
:   An algorithm for traversing or searching tree or graph data structures. The algorithm starts
at the root node and explores as far as possible along each branch recursively. See [Tree DFS](#intro-tree-dfs) for tree-specific implementation and [Matrix DFS](#intro-matrix-dfs) for matrix-specific implementation.

### Breadth First Traversal {#glossary-bfs}
...
depth level. See [Tree BFS](#intro-tree-bfs) for tree-specific implementation and [Matrix BFS](#intro-matrix-bfs) for matrix-specific implementation.
    
### Topological Sort {#glossary-topological}
...
**See also:** [Detailed explanation in Common Strategies](\@ref(backtracking))

### Dynamic Programming {#glossary-dynamic-programming}
...
**See also:** [Detailed explanation in Common Strategies](\@ref(dynamic-programming))

### Memoization {#glossary-memoization}
...
**See also:** [Detailed explanation in Common Strategies](\@ref(dp-memoization))

### Tabulation {#glossary-tabulation}
...
**See also:** [Detailed explanation in Common Strategies](\@ref(dp-tabulation))

### Divide and Conquer {#glossary-dnc}
...
### Binary Search Tree {#glossary-bst}
...
**See also:** [Detailed explanation in Common Strategies](\@ref(sorting-merge-sort))

### Bubble sort {#glossary-bs}
...
**See also:** [Detailed explanation in Common Strategies](\@ref(sorting-bubble-sort))

### Insertion Sort {#glossary-insertion-sort}
...
**See also:** [Detailed explanation in Common Strategies](\@ref(sorting-insertion-sort))

### Quick Sort {#glossary-qs}
:   a Divide and Conquer algorithm. It picks an element as pivot and partitions the given array around the
    picked pivot.

**See also:** [Detailed explanation in Common Strategies](\@ref(sorting-quick-sort))

### Minimum Heap {#glossary-heap}
...
### Two Pointer {#glossary-two-pointer}
...
**See also:** [Detailed explanation in Common Strategies](\@ref(two-pointer))

### Sliding Window {#glossary-sliding-window}
...
**See also:** [Two Pointer technique (Pattern 3: Sliding Window)](\@ref(two-pointer))

### Hash Table {#glossary-hash-table}
...
### Binary Search {#glossary-binary-search}
:   a search algorithm that finds the position of a target value within a sorted array by repeatedly dividing the search interval in half. Time complexity is O(log n). See [Binary Search](#binary-search-strategy) for detailed implementation comparison (iterative vs recursive).
...
### Matrix Traversal {#glossary-matrix-traversal}
:   a technique for visiting each element in a 2D matrix systematically, typically in patterns such as row-by-row, column-by-column, diagonal, or spiral order. See [Matrix Traversal Patterns](#intro-matrix-traversal) for detailed explanations, [Matrix DFS](#intro-matrix-dfs), and [Matrix BFS](#intro-matrix-bfs).

### Matrix Manipulation {#glossary-matrix-manipulation}
...
### Linked List {#glossary-linked-list}
...
### Bitwise Operations {#glossary-bitwise}
...
### Rejection Sampling {#glossary-rejection-sampling}
...
### Reservoir Sampling {#glossary-reservoir-sampling}
