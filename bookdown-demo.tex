...
was built on top of R Markdown and \textbf{knitr} \citep{bookmark}.

\chapter{Literature}\label{literature}

Here is a review of existing methods.

\section{Two Sum IV - Input is a BST / LeetCode 653 /
Easy}\label{two-sum-iv---input-is-a-bst-leetcode-653-easy}
...
\subsubsection{Walkthrough\}\textbackslash{}}\label{walkthrough}

Use a HashSet to store value of current node, that is (node.val). Then
check if answer = (target - node.val) and return true if existed;
otherwise, recursively call the same function for its left and right
children.

\subsubsection{Analysis}\label{analysis}

Time complexity is O(n) since every node is visited. Auxiliary Space is
O(1)
