...
5. Call helper method `isPalindrome()` with the filtered string:
   ...
   - While left pointer $\le$ right pointer, compare characters at both positions:
...
5. **Helper function `lengthOfLongestSubstring`**: Implements the standard sliding window algorithm:
   - Initialize `HashSet<Character> seen` to track characters in current window
   - Use two pointers: `l` (left) and `r` (right) to define the window
   - Expand window by moving `r`: Add each character to the set
   - When duplicate found: Shrink window from `l` until duplicate is removed
   - Track maximum window size using `r - l + 1`
